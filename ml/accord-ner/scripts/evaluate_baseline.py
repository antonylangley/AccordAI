...
def exact_entity_set(entities: list[dict]) -> set[tuple[int, int]]:
    return {(int(e["start"]), int(e["end"])) for e in entities if e["label"] == "PERSON"}


def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate the untouched DistilBERT NER baseline on Accord PERSON data.")
    parser.add_argument("dataset", type=Path)
    parser.add_argument("--model", default=MODEL_NAME)
    parser.add_argument("--output", type=Path, default=Path("results/baseline_errors.json"))
    ...
    for row in rows:
        ...
        for prediction in predictions:
            ...
            predicted_people.append(
                {
                    "text": row["text"][prediction["start"]:prediction["end"]],
        ...
        pred = {(p["start"], p["end"]) for p in predicted_people}
            ...
            errors.append(
                {
                    ...
                    "false_positive_spans": sorted(list(pred - gold)),
                    "false_negative_spans": sorted(list(gold - pred)),
                }
            )
